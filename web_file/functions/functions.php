...
function get_posts(){
							...
							<div class='panel panel-info'>
								<div class='panel-body'>
									<form action='' method='post' class='form-inline'>
										<textarea placeholder='Write your comment here!' style='resize: none; padding: 20px; height: 110px; width: 100%; border: 1px solid #F2F2F2;' name='comment'></textarea>
	...
	function results_admin(){
		global $con;

		if(isset($_GET['search1'])){
			$search_query1 = htmlentities($_GET['user_query1']);
		}

		$get_posts = "select * from posts where post_content like '%$search_query1%' OR upload_image like '%$search_query1%'";
		$run_posts = mysqli_query($con,$get_posts);
				...
				<div class='row'>
					...
					<div id='posts' class='col-sm-6'>
						<div class='row'>
							...
							<div class='col-sm-6'>
								<h3><a style='text-decoration:none; cursor:pointer;color #3897f0;' href='user_view.php?u_id=$user_id'>$user_name</a></h3>
								<h4><small style='color:black;'>Updated a post on <strong>$post_date</strong></small></h4>
							</div>
							<div class='col-sm-4'>
							</div>
	...
	function search_user_admin(){
		...
		if(isset($_GET['search_user_btn1'])){
			$search_query1 = htmlentities($_GET['search_user1']);
			$get_user = "select * from users where f_name like '%$search_query1%' OR l_name like '%$search_query1%' OR user_name like '%$search_query1%'";
		}else{
			$get_user ="select * from users";
		}
		$run_user = mysqli_query($con,$get_user);
			...
			echo"
				<div class='row'>
					...
					<div class='col-sm-6'>
						<div class='row' id='find_people'>
							<div class='col-sm-4'>
								<a href='user_view.php?u_id=$user_id'>
								<img src='users/$user_image' width='150px' height='140px' title='$username' style='float:left; margin:1px;'/>
								</a>
							</div><br><br>
							<div class='col-sm-6'>
								<a style='text-decoration:none; cursor:pointer; color:#3897f0;' href='user_view.php?u_id=$user_id'>
								<strong><h2>$f_name $l_name</h2></strong>
								</a>
							</div>
							<div class='col-sm-3'>
		...
		if(isset($_GET['post_id'])){
			...
			if($p_id != $post_id){
				...
				if(isset($_POST['reply'])){
					...
					if($comment == ""){
						echo "<script>alert('Enter your comment!')</script>";
						echo "<script>window.open('single_admin.php?post_id=$post_id', '_self')</script>";
					}else{
						$insert = "insert into comments (post_id,user_id,comment,comment_author,date) values('$post_id','99','$comment','Admin',NOW())";
